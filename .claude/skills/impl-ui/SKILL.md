...
- `lib/shared/navigation/router.dart` — existing routes
...
lib/
  ...
  shared/
    ...
    navigation/      ← go_router config (router.dart) + AppRoute enum (app_route.dart)
```
...
- Run `fvm flutter analyze <file1> <file2> ...` on every file created or modified, fix any errors, then list the files and note any follow-up needed (e.g. localisation strings, assets, theme tokens)
...
- Navigation: use `context.go()` / `context.push()` from `go_router` — no `Navigator.push` unless justified; routes use the `AppRoute` enum (`lib/shared/navigation/app_route.dart`), no raw string paths
...
- Trailing commas: add a trailing comma to every constructor call or widget with 2 or more arguments — required for `fvm dart format` to expand args onto separate lines
